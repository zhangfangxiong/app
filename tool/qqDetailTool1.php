...
class qqDetailTool extends baseTool
{
    ...
    public function exportFileAction()
    {
        ...
        if (!$_POST['sExplodeNum']) {
            showError("请输入导出数目", true);
        }
        if (!$_POST['check_age']) {
            //可以不选年龄区间
            //showError("请选择生日区间或年龄区间");
        }
        ...
        if ($_POST['check_age'] == 2 && !preg_match("/^(19|20)[0-9]{2}-[0,1][1-9]-[0,3][1-9]$/ix",$_POST['startBirth'])) {
            showError("生日格式有误",true);
        }
        if ($_POST['check_age'] == 2 && $_POST['endBirth'] && !preg_match("/^(19|20)[0-9]{2}-[0,1][1-9]-[0,3][1-9]$/ix",$_POST['endBirth'])) {
            showError("生日格式有误",true);
        }
        $aData = array();
        if ($_POST['sex']) {
